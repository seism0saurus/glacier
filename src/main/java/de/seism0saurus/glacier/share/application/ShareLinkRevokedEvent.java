...
/**
 * Domain event emitted by {@link ShareLinkServiceImpl} after a share link is
 * marked revoked in the repository.
 *
 * <p>Carries only the minimum data required for routing: the sharer's wallId and
 * the raw share-link ID (SR-RELAY-11). Listeners use these two fields to remove
 * the entry from {@link ShareLinkActivityRegistry} and to push the STOMP revocation
 * control message to viewers.
 *
 ...
 * </ul>
 *
 * @param sharerWallId the wallId of the sharer who owns the revoked link
 * @param shareLinkId  the raw share-link ID; used to build STOMP topic paths
 */
record ShareLinkRevokedEvent(String sharerWallId, ShareLinkId shareLinkId) {
