...
public final class ShareLink {
    ...
    /**
     * Reconstitutes a {@link ShareLink} from durable storage.
     *
     * <p><strong>Persistence reconstitution ONLY</strong> — not a domain-creation path.
     * Use {@link #create} for all new share links. This factory is intentionally restricted
     * to callers in {@code share.infrastructure} (ArchUnit rule ADR-SQLITE-08).
     *
     * @param id           the share-link ID restored from storage; must not be null
     * @param sharerWallId the sharer's wallId; must not be null
     * @param creatorIp    the creator IP at original creation time; null is accepted for
     *                     aggregates reconstituted from SQLite (IP is stored as HMAC, not raw)
     * @param createdAt    creation instant; must not be null
     * @param expiresAt    expiry instant; must not be null
     * @param revokedAt    revocation instant; null if not revoked
