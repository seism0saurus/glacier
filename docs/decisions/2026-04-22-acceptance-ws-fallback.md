...
| + | [ADR-05](2026-04-21-planning-ws-fallback.md#adr-05---cache-eviction-coordinated-through-subscriptionmanager) `MessageCacheSubscriptionLifecycleIT` 5-min timer not implemented | Low | Fixed | `MessageCacheDisconnectTimerIT.java` — `@SpringBootTest` + `glacier.timeouts.client_reconnect=500ms` + Awaitility + `ApplicationEventPublisher.publishEvent(SessionDisconnectEvent)` | 2 scenarios: eviction after timeout; cache preserved on reconnect within timeout |
...
- [`glacier-fallback-mode-discipline`](../../.claude/skills/glacier-fallback-mode-discipline.md), [`glacier-structured-logging-logback`](../../.claude/skills/glacier-structured-logging-logback.md), [`angular-i18n-localize`](../../.claude/skills/angular-i18n-localize.md) (mandatory skills consulted throughout the three phases).
