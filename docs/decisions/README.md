...
### Secure CI-Pipeline — Trust-Tier Topology (`secure-ci-pipeline`)
| File | Summary |
|---|---|
...
| [2026-06-24-planning-share-view-toot-rendering.md](2026-06-24-planning-share-view-toot-rendering.md) | Planning: complete the deferred toot-rendering pipeline — relay still emits `CacheEntry` while `ShareRenderingService` (`ReadonlyTootView` builder) is wired only into tests, and the catalog endpoint is stubbed (empty hashtags, no `initialToots`). Wire `ShareRenderingService` → relay; add `SubscriptionManager.getSubscribedHashtags`; add catalog `initialToots`. MVP (live path) vs full (history). e2e `share-wall-roundtrip.spec.ts` is `test.fixme` pending this. |
| [2026-06-24-planning-share-view-toot-rendering-mvp.md](2026-06-24-planning-share-view-toot-rendering-mvp.md) | P1 (MVP): Approved planning ADR — lane partition for catalog vertical (tdd-ddd-implementer: `getSubscribedHashtags` + catalog wiring), relay/StompCallback (secure-tdd-implementer), and frontend FLAW-1 reactive rewire (frontend-designer). SR-SUB/SR-CAT/SR-RENDER/SR-LOG requirements recorded. |
| [2026-06-24-implementation-share-view-toot-rendering-mvp.md](2026-06-24-implementation-share-view-toot-rendering-mvp.md) | P2 (MVP): Implemented — catalog returns subscribed hashtags + empty `initialToots`; relay renders per-link `ReadonlyTootView` via `ShareRenderingService`; FLAW-2 resolved as B2 (generic streaming path parses `GenericMessage` JSON → `Status`, gates preserved); frontend FLAW-1 reactive subscribe-after-catalog. Backend 391 (full bundle 1806) + Jacoco, Karma 800; all 11 SRs confirmed. |
| [2026-06-24-sr-csp-01-follow-up.md](2026-06-24-sr-csp-01-follow-up.md) | SR-CSP-01 follow-up (open): suspected `require-trusted-types-for` violation on the share route; no injectable HTML found in static analysis; three resolution options (A: remove directive, B: Angular TT integration, C: separate AOT bundle) documented for next sprint. Runtime capture to be (re)done on the current Angular 20 build (F-6). |
| [2026-06-25-acceptance-share-view-toot-rendering-mvp.md](2026-06-25-acceptance-share-view-toot-rendering-mvp.md) | P3 (MVP): **PASSED WITH CONDITIONS** — no Critical/High; all 10 SRs map to passing run-set tests; backend `verify` green (after fixing the ADR-index sentinel) + Jacoco, Karma 800. Conditions: live-relay Playwright e2e green in CI before merge (no local end-to-end live proof; e2e stays `test.fixme`); SR-CSP-01 runtime capture on Angular 20 (tracked follow-up); FLAW-3 fallback-blank + image-proxy bearer surface accepted residuals. |
