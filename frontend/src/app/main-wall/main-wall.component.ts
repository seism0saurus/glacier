...
/**
 * Main social-wall view (the default route `''`).
 *
 * <p>This is the owner's live wall: header, hashtag subscription panel, and the
 * streaming toot wall. It was previously the body of {@link AppComponent}; it moved
 * into its own routed component when the app gained a router so that the dedicated
 * readonly share view (`/share/:id`) can render WITHOUT the main-wall chrome
 * (header / hashtag panel / footer) or its fallback wiring.
 *
 * <p>Wires the FallbackService event stream (D-16, D-17) to the user-visible
 * notification surfaces:
 *
 *   - SessionExpired  → sets `sessionExpired = true` (full-width banner, D-17).
 *   - GapObserved     → MatSnackBar "Einige ältere Toots könnten fehlen." (10 s, per-hashtag, D-16).
 *   - RateLimited     → one-shot MatSnackBar (D-17); includes a countdown when Retry-After > 60 s.
 *
 * The live-region suppression for simultaneous gap snackbar + indicator flip (D-16)
 * is handled inside ConnectionStatusComponent via the shared FallbackService.events$
 * stream, so this component does not coordinate with it directly.
 */
@Component({
...
export class MainWallComponent implements OnInit, OnDestroy {
