...
// Re-export MessageQueue so that existing tests that import it from this
// module continue to compile without modification during the strangler-fig
// refactor.  In commit 5 (facade slimming), consumers can be migrated to
// import directly from subscription-persistence.service.ts.
...
export class SubscriptionService {
  ...
  private receivedMessages: MessageQueue;
  ...
  constructor(
    ...
    // Initialise queue via persistence factory (SR-SPLIT-03: one instance only).
    ...
    // SR-SPLIT-01a: loadHashtags() is the sole authorised reader of the
    // 'hashtags' localStorage key.  It applies validateHashtagsList before
    // returning any value (OWASP A03:2021, CWE-20, FIND-P3-SEC-4).
    // TODO: Lane B — delegate to SubscriptionStompClient
  ...
  getCreatedEvents(): Observable<WallMessage[]> {
    ...
    // SR-SPLIT-01: probe via persistence to keep all localStorage access
    // to 'messageQueue' inside SubscriptionPersistence (AC-1, AC-16).
...
// MessageQueue is now defined in subscription-persistence.service.ts.
// It is re-exported from this file at the top (export {MessageQueue}) so that
// existing consumers that import MessageQueue from './subscription.service'
